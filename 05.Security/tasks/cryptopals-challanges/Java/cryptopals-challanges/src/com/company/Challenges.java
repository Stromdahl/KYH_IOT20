...
public class Challenges {
    ...
    void testCallenge1() {
        ...
        byte[] bytes = Crypto.hexStringToByteArray(input);
        System.out.println(new String(bytes));

        String actual = Crypto.base64Encode(bytes);
    ...
    void testChallenge2() {
        ...
        String keyHex =   "686974207468652062756c6c277320657965";
        String expected = "746865206b696420646f6e277420706c6179";

        byte[] inputBytes = Crypto.hexStringToByteArray(inputHex);
        byte[] keyBytes = Crypto.hexStringToByteArray(keyHex);

        byte[] actualBytes = Crypto.xorBytes(inputBytes, keyBytes);

        System.out.println(new String(keyBytes));

        String actual = Crypto.bytesToHex(actualBytes).toLowerCase(Locale.ROOT);
    ...
    @Test
    void testChallenge3() {
        String inputHex = "1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736";
        byte[] inputBytes = Crypto.hexStringToByteArray(inputHex);
        String bestString = "";
        Double bestScore = 0d;
        char bestKey = ' ';
        ...
        for (byte key = 32; key < 127; key++) {
            byte[] resultBytes = Crypto.singleCharKeyXOR(inputBytes, key);
        ...
        System.out.println("String: " + bestString);
    ...
    void testChallenge4() {
        ...
        final String[] best = {""};
        Arrays.stream(hexStrings).parallel().forEach(s -> {
            String string = Crypto.getBestEnglishString(Crypto.hexStringToByteArray(s));
            best[0] = Crypto.getEnglishScore(string) > Crypto.getEnglishScore(best[0]) ? string : best[0];
        });
        System.out.println(best[0]);
        assertEquals("Now that the party is jumping", best[0].trim());
    }
    ...
    @Test
    void testChallenge6() {
        String inputText = "HUIfTQsPAh9PE048GmllH0kcDk4TAQsHThsBFkU2AB4BSWQgVB0dQzNTTmVS\n" +
        ...
        for (int KEYSIZE = 2; KEYSIZE < 40; KEYSIZE++) {
            ...
            byte[][] chunks = Crypto.divide(inputBytes, KEYSIZE);
            for (int i = 0; i < chunks.length - 3; i += 4) {
                byte[] a = chunks[i];
                byte[] b = chunks[i + 1];
                byte[] c = chunks[i + 2];
                byte[] d = chunks[i + 3];
                double distance = ( Crypto.hammingDistance(a, b) +
                                    Crypto.hammingDistance(a, c) +
                                    Crypto.hammingDistance(a, d) +
                                    Crypto.hammingDistance(b, c) +
                                    Crypto.hammingDistance(b, d) +
                                    Crypto.hammingDistance(c, d)) / 6d;
                distances.add(Math.round(distance / KEYSIZE));
            }
            System.out.println(distances.stream().min(Long::compare).get());
        }
    }
